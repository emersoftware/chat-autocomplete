...
import CompletionEditableDiv from "@/components/CompletionEditableDiv";
...
export default function Home(this: any) {
  ...
  const { messages, input, setInput, handleSubmit, isLoading: chatEndpointIsLoading } =
    useChat({
      api: endpoint,
      onResponse() {
        setInput('');
    ...
    <main className="w-full lg:w-1/2 h-screen mx-auto text-white flex flex-col items-center justify-between p-24">
      ...
      <form onSubmit={sendMessage} className="flex w-full flex-col">
        <div className="flex flex-col space-y-2 w-full mt-2">
          <CompletionEditableDiv setChatInput={setInput} chatInput={input}>
